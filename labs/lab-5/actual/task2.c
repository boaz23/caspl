...
char *process_status_name(process *p) {
...
bool check_process_state_and_update_status(process *p) {
    ...
    if (status != UNCHANGED && status != STATUS_ERR) {
        p->status = status;
        return TRUE;
    }
...
void print_process_info(process *p) {
    char *s_status = process_status_name(p);
    if (s_status) {
        printf("%-12d %-12s", p->pid, s_status);
    }
    else {
        printf("%-12d %-12d", p->pid, p->status);
    }
