...
bool verify_before_write(FILE *f, int file_offset) {
    ...
    if (file_offset > last_file_offset) {
        printf("requested writing to file at offset %X but file's size is %X\n", file_offset, last_file_offset);
        return FALSE;
    }
...
void save_to_file(FILE *f, int file_offset, int unit_size, int count, char *buffer) {
    int written_units_to_file_amount;
    if (!verify_before_write(f, file_offset)) {
        return;
    }
    ...
    written_units_to_file_amount = fwrite(buffer, unit_size, count, f);
    if (written_units_to_file_amount < count) {
        perror("fwrite - not all written");
        return;
    }
}

void save_into_file_act(state *s) {
    ...
    get_source_buffers_bounds(s, (void*)source_address, length, &start, &end);
    if ((void*)source_address == NULL) {
        int available_bytes = end - start;
        int requested_bytes = length * s->unit_size;
        if (available_bytes != requested_bytes) {
            printf("requested writing %d bytes but the buffer only has %d\n", requested_bytes, available_bytes);
            return;
        }
    }
    ...
    save_to_file(f, target_location, s->unit_size, length, start);
    fclose(f);
}
...
void memory_modify_act(state *s) {
    ...
    last_byte_offset = location + s->unit_size;
    if (last_byte_offset >= ARR_LEN(s->mem_buf)) {
        printf("location and unit size were out of bounds of the memory buffer\n");
        return;
    }

    p_mem = s->mem_buf + location;
    switch (s->unit_size) {
        case 1:
            *((char*)p_mem) = (char)val;
            break;
        ...
        case 4:
            *((int*)p_mem) = (int)val;
            break;
        default:
            dbg_printf(s, "invalid unit size (%d) at memoty_modify\n", s->unit_size);
            break;
    }

    if (s->mem_count < last_byte_offset) {
        s->mem_count = last_byte_offset;
    }
}
...
int main(int argc, char *argv[]) {
    menu_item menu[] = {
        ...
        { "Save Into File", save_into_file_act },
